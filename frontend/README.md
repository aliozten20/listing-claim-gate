# frontend — Raw LLM Monitoring & Deci.Scoring (SPA)
...
and shows its **Deci.Scoring** result.
...
The backend ([`backend`](../backend)) must be running and its
...
Root Directory must be `frontend`. Then deploy via the Vercel MCP or
`vercel --prod`. After the frontend URL is known, update the backend's
`CORS_ORIGINS` on Render to match.
