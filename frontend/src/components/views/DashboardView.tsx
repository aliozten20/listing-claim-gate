...
export function DashboardView() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-5 space-y-5">
      ...
      {metrics?.model_efficiency && metrics.model_efficiency.length > 0 && (
        <ModelEfficiencyTable rows={metrics.model_efficiency} />
      )}
            ...
            {loading && runs.length === 0 ? (
              ...
              runs.map((r) => (
                ...
                >
                  ...
                  <div
                    className="text-xs mono mt-1 flex gap-3 flex-wrap"
                    style={{ color: "var(--text-faint)" }}
                  >
                    <span>{r.model.split("-").slice(0, 2).join(" ")}</span>
                    <span>{r.latency_ms}ms</span>
                    <span>
                      {r.latency_ms > 0 && r.completion_tokens > 0
                        ? `${((r.completion_tokens / r.latency_ms) * 1000).toFixed(0)} tok/s`
                        : "— tok/s"}
                    </span>
          ...
          <div key={g} className="flex-1 flex flex-col items-center gap-1">
            <div className="w-full flex-1 flex items-end">
              <div
                ...
                style={{ height: `${(n / max) * 100}%`, minHeight: n > 0 ? "6px" : "0" }}
              />
            </div>
