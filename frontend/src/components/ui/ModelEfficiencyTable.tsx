  ...
  return (
    <div className="card p-4">
      <div className="flex items-center justify-between mb-3">
        <div>
          <h3 className="text-sm font-semibold">Model efficiency comparison</h3>
          <p className="text-xs mt-0.5" style={{ color: "var(--text-faint)" }}>
            Average throughput and Deci.Scoring efficiency by model
          </p>
        </div>
      </div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr style={{ color: "var(--text-faint)" }}>
              ...
              <th className="text-right font-medium py-2 px-2">Runs</th>
              ...
              <th className="text-right font-medium py-2 px-2">Eff.</th>
          ...
          <tbody>
            {rows.map((r) => (
              ...
              >
                <td className="py-2.5 pr-3 mono truncate max-w-[14rem]" title={r.model}>
                  {shortModel(r.model)}
                </td>
                <td className="text-right mono px-2">{r.runs}</td>
                <td className="text-right mono px-2" style={{ color: tokColor(r.avg_tokens_per_sec) }}>
                  {r.avg_tokens_per_sec > 0 ? r.avg_tokens_per_sec.toFixed(1) : "—"}
                </td>
                <td className="text-right mono px-2">
                  {r.avg_chars_per_token > 0 ? r.avg_chars_per_token.toFixed(1) : "—"}
                </td>
                <td className="text-right mono px-2">
                  {Math.round(r.avg_latency_ms)}ms
                </td>
                <td className="text-right mono px-2" style={{ color: barColor(r.avg_efficiency) }}>
                  {r.avg_efficiency > 0 ? r.avg_efficiency.toFixed(0) : "—"}
                </td>
                <td className="text-right mono pl-2" style={{ color: "var(--accent)" }}>
...
function shortModel(id: string): string {
  return id.replace(/-MLC$/, "").replace(/-Instruct/gi, "").replace(/-it/gi, "");
}
