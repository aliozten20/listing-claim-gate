...
func NewAuthStore(db *pgxpool.Pool) *AuthStore { return &AuthStore{db: db} }
...
func (s *AuthStore) CreateUser(ctx context.Context, email, passwordHash, name string) (auth.User, error) {
	var u auth.User
...
func (s *AuthStore) GetUserByEmailWithHash(ctx context.Context, email string) (auth.User, string, error) {
	var u auth.User
	var hash string
	err := s.db.QueryRow(ctx,
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return auth.User{}, "", auth.ErrNoRows
	}
	return u, hash, err
}

// GetUserByID returns a user by id.
func (s *AuthStore) GetUserByID(ctx context.Context, id string) (auth.User, error) {
	var u auth.User
	err := s.db.QueryRow(ctx,
...
func (s *AuthStore) GetPasswordHash(ctx context.Context, id string) (string, error) {
	var hash string
...
func (s *AuthStore) UpdateName(ctx context.Context, id, name string) (auth.User, error) {
	var u auth.User
	err := s.db.QueryRow(ctx,
...
func (s *AuthStore) UpdatePassword(ctx context.Context, id, newHash string) error {
	_, err := s.db.Exec(ctx,
...
func (s *AuthStore) CreateSession(ctx context.Context, userID, tokenHash, userAgent, ip string, expires time.Time) (string, error) {
	var id string
	err := s.db.QueryRow(ctx,
...
func (s *AuthStore) FindValidSessionByHash(ctx context.Context, tokenHash string) (sessionID, userID string, err error) {
	err = s.db.QueryRow(ctx,
...
func (s *AuthStore) FindSessionByHashAnyState(ctx context.Context, tokenHash string) (auth.SessionLookup, error) {
	var out auth.SessionLookup
	err := s.db.QueryRow(ctx,
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return auth.SessionLookup{}, auth.ErrNoRows
	}
	return out, err
}
...
func (s *AuthStore) RevokeAllSessionsForUser(ctx context.Context, userID string) (int64, error) {
...
func (s *AuthStore) RevokeSessionForUser(ctx context.Context, sessionID, userID string) error {
	tag, err := s.db.Exec(ctx,
...
func (s *AuthStore) ListSessions(ctx context.Context, userID string) ([]auth.Session, error) {
	...
	sessions := make([]auth.Session, 0, 8)
	for rows.Next() {
		var se auth.Session
