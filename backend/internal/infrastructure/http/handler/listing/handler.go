...
type RunStore interface {
	CreateRun(ctx context.Context, userID string, req listing.CreateRunRequest) (listing.Run, error)
	GetRun(ctx context.Context, userID, runID string) (listing.Run, error)
	ListRuns(ctx context.Context, userID, model string, limit int, before time.Time) (listing.ListResult, error)
	DeleteRun(ctx context.Context, userID, runID string) error
	UpsertScore(ctx context.Context, sc listing.Score) (listing.Score, error)
	Metrics(ctx context.Context, userID string) (listing.Metrics, error)
}
...
// SetMLC attaches an optional worker MLC client (tunnel via MLC_BASE_URL).
func (h *Handler) SetMLC(c mlc.Client) {
	h.mlc = c
}
...
var browserModels = []listing.ModelInfo{
...
func (h *Handler) CreateRun(w http.ResponseWriter, r *http.Request) {
	...
	var req listing.CreateRunRequest
	...
	if req.AutoScore {
		score := listingapp.ScoreRun(run, listingapp.DefaultWeights())
...
func (h *Handler) ScoreRun(w http.ResponseWriter, r *http.Request) {
	...
	weights := listingapp.DefaultWeights()
	if req.Weights != nil {
		weights = *req.Weights
	}
	score := listingapp.ScoreRun(run, weights)
