...
type Score struct {
	...
	EfficiencyAnalysis *EfficiencyReport `json:"efficiency_analysis,omitempty"`
	CreatedAt          time.Time         `json:"created_at"`
}
...
// Weights control how much each Deci.Scoring component contributes (sum to 1.0).
type Weights struct {
...
type Metrics struct {
	...
	ModelEfficiency    []ModelEfficiency `json:"model_efficiency"`
}
...
// ErrNoRows is returned when a listing/run lookup finds nothing.
